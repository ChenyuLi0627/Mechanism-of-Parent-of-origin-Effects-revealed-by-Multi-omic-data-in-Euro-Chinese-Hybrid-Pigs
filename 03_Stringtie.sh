...
input=/work/LCY_tmp/RNAseq/phased
output=/home/Lichenyu/expand_five_RNAseq/phase
...
##################                                                   ##########
###############################################################################
inputfile=`cat ${output}/phase_sample.txt|head -${PBS_ARRAYID}|tail -1`
echo "$inputfile" |sed 's/.bam//' |awk -F/ '{print $NF}'
outputfile=`echo "$inputfile" | sed 's/.bam//' |awk -F/ '{print $NF}'`
/home/PersonalFiles/rnaqc/soft/stringtie-master/stringtie   ${input}/${inputfile} --rf -p 10 -o ${outputfile}_transcript.gtf -A ${outputfile}.tsv  -G /home/Lichenyu/ref/DRCv19_mother_DRCv20_liftoff.sorted.gtf -e
