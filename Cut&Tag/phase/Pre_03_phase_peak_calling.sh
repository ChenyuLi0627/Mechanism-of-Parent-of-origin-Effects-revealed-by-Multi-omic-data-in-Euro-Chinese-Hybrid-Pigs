...
##################################################################
cd ${tmpdir}/${Work_dir}
inputfile=`cat ${input}/sample_phase.txt|head -${PBS_ARRAYID}|tail -1`
echo "$inputfile" |sed 's/.bam//' |awk -F/ '{print $NF}'
outputfile=`echo "$inputfile" | sed 's/.bam//' |awk -F/ '{print $NF}'`
histone=`echo $outputfile |awk -F "_" '{print $1}'`

singularity  exec ~/../guilu/software/142.macs.sif macs2 callpeak -f BAMPE  -t /home/gemeichenyu/transfer/cuttag/bin_coverage/${outputfile}.bam  -n ${outputfile} -g 2682231099 --outdir /home/gemeichenyu/transfer/cuttag/bin_coverage  --bdg -q 0.05  --nomodel --shift -0  --keep-dup all
